...
<html lang="es">
    ...
    <body>
        <div class="lead" style="font-size: 15px;">
            <small>
                <a href="">Inicio</a> > <a href="">Materiales y Herramientas</a> > <a href="">Consultar Materiales</a>
                <hr style="background-color: gold; width: 100%">
            </small>
        </div>
        <div class="container-fluid">
            <h1 class="h2 text-center my-5" style="font-family: Microsoft YaHei Light">CONSULTAR MATERIALES</h1>
            <br>
            <a class="btn btn-success" style="color: #fff"  data-toggle="modal" data-target="#modalAgregar">Agregar Nuevo</a>
            <hr>
            <table id="example" border="1" class="table col-md-12 table-condensed  table-hover table-striped table-bordered text-center">
                                    ...
                                    <form id="frm_Nuevo">
                                        <center>
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="txtNombre" placeholder="Material" required>
                                            </div>    
                                            ...
                                            <div class="form-group"> 
                                                ...
                                                <input type="text" class="form-control" name="txtCantidad" placeholder="Cantidad" required>
                                            </div>

                                            <div class="form-group"> 
                                                <label for="" class="control-label">Marca</label>
                                                <input type="text" class="form-control" name="txtMarca_Id_marca" placeholder="Marca del Material" required>
                                            </div>

                                            <div class="form-group"> 
                                                <label for="" class="control-label">Tipo de Material</label>
                                                <input type="text" class="form-control" name="txtTipo_materiales_Id_tipo_materiales" placeholder="Tipo de Material" required>
                                            </div>
                                    ...
                                    <form id="frm_Actualizar">
                                        <center>
                                            <div class="form-group">
                                                ...
                                                <input type="number" readonly="readonly" id="IdHerramienras" style="background:rgba(0,0,0,0.1)" class="form-control" name="txtId_materiales"  placeholder="Id">    
                                            </div>

                                            <div class="form-group">
                                                <label for="" class="control-label">Material</label>
                                                <input type="text" class="form-control" id="material" name="txtNombre" placeholder="Material">
                                            </div>    

                                            <div class="form-group"> 
                                                <label for="" class="control-label">Descripción</label>
                                                <input type="text" class="form-control" id="descripcion" name="txtDescripcionMtl"  placeholder="Descripción">
                                            </div>
                                            <div class="form-group"> 
                                                <label for="" class="control-label">Precio</label>
                                            ...
                                            <div class="form-group"> 
                                                <label for="" class="control-label">Marca</label>
                                                <input type="text" class="form-control" id="marca" name="txtMarca_Id_marca"  placeholder="Marca del Material">
                                            </div>

                                            <div class="form-group"> 
                                                <label for="" class="control-label">Tipo de Material</label>
                                                <input type="text" class="form-control" id="tipoHerramienta" name="txtTipo_materiales_Id_tipo_materiales"  placeholder="Tipo de Herramienta">
                                            </div>                         
                                            <div class="modal-footer">
                                                <button id="limpiarrenidre"type="reset" class="btn btn-outline-info"  >
                                                    Limpiar
        ...
        <script>
                ...
                var cantidad = function () {
                    var data = "";
                    $.ajax({
                        url: "<%=request.getContextPath()%>/Administrador?accion=alertaImpelentoEscosos",
                        type: "POST",
                        data: data,
                        dataSrc: "datos",
                        dataType: "json",
                    ...
                    var data = "";
                    $.ajax({
                        url: "<%=request.getContextPath()%>/Administrador?accion=ListarImplementosEscasos",
                        type: "POST",
                        data: data,
                        dataSrc: "datos",
                        dataType: "json",
                ...
                var listar = function () {
                    var table = $("#example").DataTable({
                        ...
                        columns: [
                            //{data: "getId_materiales"},
                            {data: "getNombre"},
                            {data: "getDescripcion"},
                            {data: "getPrecio"},
                            {data: "getCantidad"},
                            {data: "getMarca_Id_marca"},
                            {data: "getTipo_materiales_Id_tipo_materiales"},
                            {data: "sumarCantidad"},
                            {data: "Editar"},
                            {data: "Eliminar"}
                        ],
        ...
        <script>
            ...
            $('#btn_registrar').click(function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<%=request.getContextPath()%>/Administrador?accion=AgregarMateriales",
                    data: datos,
            ...
            $('#btn_Actualizar').click(function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<%=request.getContextPath()%>/Administrador?accion=btnActualizarHtaMateriales",
                    data: datos,
                    success: function (r) {
                        if (r == 1) {
                            Swal.fire({
            ...
            $(document).on('click', 'a.btnEditar', function (e) {
                ...
                $('#material').val(fila.find('td:eq(1)').text());
                $('#descripcion').val(fila.find('td:eq(2)').text());
                $('#precio').val(fila.find('td:eq(3)').text());
                $('#cantidad').val(fila.find('td:eq(4)').text());
                $('#marca').val(fila.find('td:eq(5)').text());
                $('#tipoHerramienta').val(fila.find('td:eq(6)').text());
            });

            $(document).on('click', 'a.btnEliminar', function (e) {
                var Id_materiales = $(this).attr('id');
                var fila = $(this).parent().parent();
                e.preventDefault();
                var data = {Id_materiales: Id_materiales};
                ...
                swalWithBootstrapButtons.fire({
                    title: '¿Desea eliminar el registro con el ID ' + Id_materiales + ' ? ',
                    type: 'warning',
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "<%=request.getContextPath()%>/Administrador?accion=eliminarMateriales",
                            type: "POST",
                            data: data,
                            success: function (data) {
                                if (data == 1) {
            ...
            $(document).on('click', 'a.anadirSumaaaaaa', function (e) {
                ...
                $('#anadirSumaaaaaa').click(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/Administrador?accion=sumarImplementos&idMaterial=" + idMaterial + "",
                        data: datoss,
                        success: function (data) {
                            if (data == 1) {
                                setTimeout(function () {
...
////*******  FUNCCION AJAX PARA QUITAR LOS IMPLEMENTO.      *************************************************************************************
            $(document).on('click', 'a.btn_QuitarImpl', function (e) {
                ...
                $('#QuitarImplement').click(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/Administrador?accion=restarImplemento&idMaterial=" + idMaterial + "",
                        data: datoss,
                        success: function (data) {
                            if (data == 1) {
                                setTimeout(function () {
